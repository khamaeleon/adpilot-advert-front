 ...
 * @type {Atom<unknown>}
 */
//export const pixelDetailDataAtom = atom(null)
...
export const pixelColumns = [
  {
    ...
    // render: (props) => {
    //   return (
    //     <Link to={'/board/pixelDetail'} state={{id: props.data.userId}}>{props.value}</Link>
    //   )
    // }
  },
  {
    ...
    render: ({value, cellProps}) => {
      return (
        <div style={{display: "flex", alignItems: 'center'}}>
          <p>{value}</p>
          <Icon icon={'script'} value={value} cellProps={cellProps}/>
        </div>
      )
    }
  }
