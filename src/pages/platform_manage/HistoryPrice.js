...
const option = [
  ...
  {key: 2, value: 'eventPriceGroup', label: '이벤트 단가 그룹명'},
...
const columns = [
  ...
  {
    name: 'eventPriceGroup',
    header: '이벤트 단가 그룹명',
    render: ({value, cellProps}) => {
      return (
        <Link to={'/board/historyPriceDetail'} state={cellProps.id}>{value}</Link>
      )
    }
  },
  {
  ...
  const dataSource = [
    {
      name:'나이키',
      id: 'Nike12',
      eventPriceGroup: '쇼핑 극대화 그룹',
      date: 'YYYY.MM.DD HH:MM',
      changer: 'gildong12@macor.com'
    }
  ]
